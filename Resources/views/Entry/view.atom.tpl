...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{route name='mueternizermodule_entry_display'  id=$items[0].id lct=$lct}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
<link rel="alternate" type="text/html" hreflang="{lang}" href="{route name='mueternizermodule_entry_index' absolute=true}" />
...
{foreach item='entry' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{route name='mueternizermodule_entry_display'  id=$entry.id lct=$lct absolute=true}" />
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$entry.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{route name='mueternizermodule_entry_display'  id=$entry.id lct=$lct}{/capture}
        <id>{$uniqueID}</id>
