...
namespace MU\EternizerModule\Entity\Repository\Base;
...
class Entry extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('index', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('entry', 'workflowState');
            }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MU\EternizerModule\Entity\EntryEntity', 'tbl');
        $query = $qb->getQuery();
        ...
        $logger->debug('{app}: User {user} truncated the {entity} entity table.', array('app' => 'MUEternizerModule', 'user' => UserUtil::getVar('uname'), 'entity' => 'entry'));
    }
    /**
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\EternizerModule\Entity\EntryEntity', 'tbl')
        ...
        $serviceManager = ServiceUtil::getManager();
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} updated {entities} created by user id {userid}.', array('app' => 'MUEternizerModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'entries', 'userid' => $userId));
    }
    ...
    public function updateLastEditor($userId, $newUserId)
    {
        ...
        $serviceManager = ServiceUtil::getManager();
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} updated {entities} edited by user id {userid}.', array('app' => 'MUEternizerModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'entries', 'userid' => $userId));
    }
    ...
    public function deleteByCreator($userId)
    {
        ...
        $serviceManager = ServiceUtil::getManager();
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} deleted {entities} created by user id {userid}.', array('app' => 'MUEternizerModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'entries', 'userid' => $userId));
    }
    ...
    public function deleteByLastEditor($userId)
    {
        ...
        $serviceManager = ServiceUtil::getManager();
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} deleted {entities} edited by user id {userid}.', array('app' => 'MUEternizerModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'entries', 'userid' => $userId));
    }
     ...
     *
     * @return array|MU\EternizerModule\Entity\EntryEntity retrieved data array or MU\EternizerModule\Entity\EntryEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MU\EternizerModule\Entity\EntryEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            ...
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                $session->set('MUEternizerModuleEntriesCurrentPage', $page);
            } else {
                // restore current page from session
                $page = $session->get('MUEternizerModuleEntriesCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUEternizerModule', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'entry', 'catids' => $v));
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.obj_status LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.ip LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
