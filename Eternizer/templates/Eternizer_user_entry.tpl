...
<div class="etz_entry z-clearfix {cycle values='etz_bg1,etz_bg2'}" >
    <div class="etz_author">
        ...
        <dl class="etz_options">
            ...
            <dd>
                ...
                <a href="{pnmodurl modname=Eternizer type=admin func=modify id=$id}" title="{gt text="Comment entry"}">{img modname=core src=comment.png set='icons/extrasmall' __title="Comment entry" __alt="Comment entry"}</a>
                {/if}
                {if $right_edit}
                <a href="{pnmodurl modname=Eternizer type=admin func=modify id=$id}" title="{gt text="Edit entry"}">{img modname=core src=edit.png set='icons/extrasmall' __title="Edit entry" __alt="Edit entry"}</a>
                {/if}
                {if $right_delete}
                <a href="{pnmodurl modname=Eternizer type=admin func=suppress id=$id}" title="{gt text="Delete entry"}">{img modname=core src=14_layer_deletelayer.png set='icons/extrasmall' __title="Delete entry" __alt="Delete entry"}</a>
                {/if}
            </dd>
            {/if}
        </dl>
    ...
    <div class="etz_body">
        <div class="etz_info">
            <strong class="etz_title">{$profile[$config.titlefield]|safetext} ({$cr_date|pndate_format:datetimebrief})</strong>
        </div>
