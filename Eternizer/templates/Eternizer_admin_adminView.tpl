    ...
    <div class="etz_entry z-clearfix {cycle values='etz_bg1,etz_bg2'}" >
        <div class="etz_author">
            <dl class="etz_options">
                ...
                <dd>
                    ...
                    <a href="{modurl modname=Eternizer type=admin func='changeStatus' status="A" id=$item.id goback=$goback}">{pnimg modname='core' src='greenled.gif' set='icons/extrasmall' __title="Activate entry" __alt="Activate entry"}</a>
                    {else}
                    <a href="{modurl modname=Eternizer type=admin func='changeStatus' status="M" id=$item.id goback=$goback}">{pnimg modname='core' src='redled.gif' set='icons/extrasmall' __title="Moderate entry" __alt="Moderate entry"}</a>
                    {/if}
                    {if $item.rights.modify}
                    <a href="{modurl modname=Eternizer type=admin func='modify' id=$item.id goback=$goback}">{pnimg modname='core' src='edit.gif' set='icons/extrasmall' __title="Edit entry" __alt="Edit entry"}</a>
                    {/if}
                    {if $item.rights.delete}
                    <a href="{modurl modname=Eternizer type=admin func='suppress' id=$item.id goback=$goback}">{pnimg modname='core' src='14_layer_deletelayer.gif' set='icons/extrasmall' __title="Delete entry" __alt="Delete entry"}</a>
                    {/if}
                </dd>
            </dl>
        </div>
        ...
        <div class="etz_body">
            ...
            <div class="etz_content">
                {$item.text|safetext|nl2br|pnmodcallhooks}
                {if $item.comment}
