...
function Eternizer_userapi_WriteEntry($args)
{
    ...
    if (($spammode > 0 && !pnUserLoggedIn()) || ($spammode > 2)) {
        $spamscan = Eternizer_antispam::scan(array(
            'text' => $args['text'],
            'profile' => $attr));
    }
    if ($spamscan == true) {
        if (($spammode == 1 && !pnUserLoggedIn()) || $spammode == 3) {
            $args['obj_status'] = 'M';
            LogUtil::registerStatus(__('Your entry is detected as potential spam.', $dom));
            ...
            LogUtil::registerStatus(__('Your entry is detected as potential spam.', $dom));
            return LogUtil::registerError(__('Your entry is rejected!', $dom));
        }
    }
