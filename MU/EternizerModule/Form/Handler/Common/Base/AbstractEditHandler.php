...
namespace MU\EternizerModule\Form\Handler\Common\Base;
...
use MU\EternizerModule\Helper\HookHelper;
use MU\EternizerModule\Helper\ModelHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        // default to referer
        $refererSessionVar = 'mueternizermodule' . $this->objectTypeCapital . 'Referer';
        ...
        $this->permissionComponent = 'MUEternizerModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUEternizerModule' . $this->objectTypeCapital . $entity->getKey();
                    $this->lockingApi->addLock($lockName, $this->getRedirectUrl(null));
                }
            }
        } else {
            $permissionLevel = in_array($this->objectType, ['entry']) ? ACCESS_COMMENT : ACCESS_EDIT;
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUEternizerModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $routeUrl = new RouteUrl('mueternizermodule_' . $this->objectTypeLower . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUEternizerModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('MUEternizerModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUEternizerModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUEternizerModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
