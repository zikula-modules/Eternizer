...
abstract class AbstractEntryController extends AbstractController
{
    ...
    protected function displayInternal(Request $request, EntryEntity $entry, $isAdmin = false)
    {
        ...
        $instanceId = $entry->createCompositeIdentifier();
        ...
        if ($format == 'ics') {
            $fileName = $objectType . '_' . (property_exists($entry, 'slug') ? $entry['slug'] : $entry->getTitleFromDisplayPattern()) . '.ics';
    ...
    protected function deleteInternal(Request $request, EntryEntity $entry, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUEternizerModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'entry', 'id' => $entry->createCompositeIdentifier()];
        ...
        $form = $this->createForm('MU\EternizerModule\Form\DeleteEntityType', $entry);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('mu_eternizer_module.hook_helper');
                // Let any hooks perform additional validation actions
                $validationHooksPassed = $hookHelper->callValidationHooks($entry, 'validate_delete');
                if ($validationHooksPassed) {
                    ...
                    $hookHelper->callProcessHooks($entry, 'process_delete', null);
