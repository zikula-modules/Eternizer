 ...
 */
function mUEternizerInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('mueternizermodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
...
function mUEternizerTriggerFormValidation()
{
    mUEternizerExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function mUEternizerHandleFormSubmit (event) {
    if (triggerValidation) {
        mUEternizerTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function mUEternizerInitEditForm(mode, entityId)
{
    if (jQuery('.mueternizer-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.mueternizer-edit-form').first();
    ...
    editForm.submit(mUEternizerHandleFormSubmit);
